...
func NewLocalTrack(logger logger.Logger, c webrtc.RTPCodecCapability, opts ...LocalTrackOptions) (*LocalTrack, error) {
	s := &LocalTrack{logger: logger}
	for _, o := range opts {
		o(s)
	}
	rid := ""
...
func NewLocalSampleTrack(logger logger.Logger, c webrtc.RTPCodecCapability, opts ...LocalTrackOptions) (*LocalTrack, error) {
	return NewLocalTrack(logger, c, opts...)
}

// SetLogger overrides default logger.
func (s *LocalTrack) SetLogger(l logger.Logger) {
	s.logger = l
}
...
func (s *LocalTrack) rtcpWorker(rtcpReader interceptor.RTCPReader) {
	...
	for {
		...
		if err != nil {
			s.logger.Warnw("could not unmarshal rtcp", err)
			return
		}
...
func (s *LocalTrack) writeWorker(provider SampleProvider, onComplete func()) {
	...
	for {
		...
		if err != nil {
			s.logger.Errorw("could not get sample from provider", err)
			return
		}
		...
		if !s.muted.Load() {
			...
			if err := s.WriteSample(sample, opts); err != nil {
				s.logger.Errorw("could not write sample", err)
				return
			}
		}
