...
import (
	...
	lrpc "github.com/livekit/livekit-server/pkg/rpc"
...
func (pri *ParticipantRelayInit) ToStartRelaySession(roomName livekit.RoomName, connectionID livekit.ConnectionID) (*lrpc.StartRelaySession, error) {
	claims, err := json.Marshal(pri.Grants)
	if err != nil {
		return nil, err
	}

	srs := &lrpc.StartRelaySession{
		RoomName:      string(roomName),
...
func ParticipantInitFromStartRelaySession(srs *lrpc.StartRelaySession, region string) (*ParticipantRelayInit, error) {
	claims := &auth.ClaimGrants{}
	if err := json.Unmarshal([]byte(srs.GrantsJson), claims); err != nil {
		return nil, err
	}

	pri := &ParticipantRelayInit{
		RoomName:        livekit.RoomName(srs.RoomName),
		ID:              livekit.ParticipantID(srs.ParticipantId),
		Identity:        livekit.ParticipantIdentity(srs.Identity),
		Name:            livekit.ParticipantName(srs.Name),
		Reconnect:       srs.Reconnect,
		ReconnectReason: srs.ReconnectReason,
		Client:          srs.Client,
		AutoSubscribe:   srs.AutoSubscribe,
		Grants:          claims,
		Region:          region,
		AdaptiveStream:  srs.AdaptiveStream,
		DisableICELite:  srs.DisableIceLite,
	}
	if srs.SubscriberAllowPause != nil {
		subscriberAllowPause := *srs.SubscriberAllowPause
		pri.SubscriberAllowPause = &subscriberAllowPause
	}
